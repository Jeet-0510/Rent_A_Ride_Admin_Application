...
public class BikeDetail extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Image = findViewById(R.id.bike_image);
        saveButton = findViewById(R.id.bike_save);
        name = findViewById(R.id.bike_name);
        desc = findViewById(R.id.bike_desc);
        location = findViewById(R.id.bike_location);
        Ruppes = findViewById(R.id.bike_rupees);
        ...
        ActivityResultLauncher<Intent> activityResultLauncher = registerForActivityResult(new ActivityResultContracts.StartActivityForResult(), new ActivityResultCallback<ActivityResult>() {
            ...
            public void onActivityResult(ActivityResult result) {
                if (result.getResultCode() == Activity.RESULT_OK) {
                    ...
                    Toast.makeText(BikeDetail.this, "No Image Selected", Toast.LENGTH_LONG).show();
                }
            }
        });
    ...
    public void uploadData() {
        ...
        BikeData bikeData = new BikeData(Cname, Cdesc, CLocation, Cruppes, url);
        String currdate = DateFormat.getDateTimeInstance().format(Calendar.getInstance().getTime());

        FirebaseDatabase.getInstance().getReference("Admin").child(mobileNumber).child("BIKE").child(currdate).setValue(bikeData).addOnCompleteListener(new OnCompleteListener<Void>() {
            @Override
            public void onComplete(@NonNull Task<Void> task) {
                if (task.isSuccessful()) {
                    Toast.makeText(BikeDetail.this, "Saved Successful", Toast.LENGTH_SHORT).show();
                }
            }
        }).addOnFailureListener(new OnFailureListener() {
            @Override
            public void onFailure(@NonNull Exception e) {
                Toast.makeText(BikeDetail.this, e.getMessage().toString(), Toast.LENGTH_LONG).show();
